...
public class SimpleExoPlayer extends BasePlayer implements Player {
    ...
    protected SimpleExoPlayer(Context context, RenderersFactory renderersFactory, TrackSelector trackSelector, LoadControl loadControl, DrmSessionManager<FrameworkMediaCrypto> drmSessionManager, BandwidthMeter bandwidthMeter2, AnalyticsCollector.Factory factory, Clock clock, Looper looper) {
        ...
        ExoPlayerImpl exoPlayerImpl2 = new ExoPlayerImpl(createRenderers, trackSelector, loadControl, bandwidthMeter2, clock, looper);
        ...
        addListener(componentListener2);
        ...
        bandwidthMeter3.addEventListener(handler, createAnalyticsCollector);
        DrmSessionManager<FrameworkMediaCrypto> drmSessionManager3 = drmSessionManager;
        if (drmSessionManager3 instanceof DefaultDrmSessionManager) {
            ((DefaultDrmSessionManager) drmSessionManager3).addListener(handler, createAnalyticsCollector);
        }
        this.audioFocusManager = new AudioFocusManager(context, componentListener2);
    }
        ...
        if (!Util.areEqual(this.audioAttributes, audioAttributes2)) {
            ...
            for (Renderer renderer : this.renderers) {
                if (renderer.getTrackType() == 1) {
                    ...
                    createMessage.setPayload(audioAttributes2);
                    createMessage.send();
                }
            }
    ...
    private void removeSurfaceCallbacks() {
        ...
        if (textureView2 != null) {
            if (textureView2.getSurfaceTextureListener() != this.componentListener) {
                Log.m16w("SimpleExoPlayer", "SurfaceTextureListener already unset or replaced.");
            } else {
    ...
    private void verifyApplicationThread() {
        if (Looper.myLooper() != getApplicationLooper()) {
            Log.m17w("SimpleExoPlayer", "Player is accessed on the wrong thread. See https://exoplayer.dev/troubleshooting.html#what-do-player-is-accessed-on-the-wrong-thread-warnings-mean", this.hasNotifiedFullWrongThreadWarning ? null : new IllegalStateException());
