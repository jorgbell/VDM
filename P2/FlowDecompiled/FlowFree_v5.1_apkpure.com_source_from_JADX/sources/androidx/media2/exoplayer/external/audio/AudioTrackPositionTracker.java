...
final class AudioTrackPositionTracker {
    ...
    public long getCurrentPositionUs(boolean z) {
        ...
        AudioTimestampPoller audioTimestampPoller3 = audioTimestampPoller2;
        if (audioTimestampPoller3.hasTimestamp()) {
            long framesToDurationUs = framesToDurationUs(audioTimestampPoller3.getTimestampPositionFrames());
            if (!audioTimestampPoller3.isTimestampAdvancing()) {
                return framesToDurationUs;
            }
            return framesToDurationUs + (nanoTime - audioTimestampPoller3.getTimestampSystemTimeUs());
        }
    ...
    public boolean mayHandleBuffer(long j) {
        ...
        if (z && !hasPendingData && playState != 1 && (listener2 = this.listener) != null) {
            listener2.onUnderrun(this.bufferSize, C0200C.usToMs(this.bufferSizeUs));
        }
        return true;
    }
    ...
    private void maybePollAndCheckTimestamp(long j, long j2) {
        ...
        Assertions.checkNotNull(audioTimestampPoller2);
        AudioTimestampPoller audioTimestampPoller3 = audioTimestampPoller2;
        if (audioTimestampPoller3.maybePollTimestamp(j)) {
            long timestampSystemTimeUs = audioTimestampPoller3.getTimestampSystemTimeUs();
            long timestampPositionFrames = audioTimestampPoller3.getTimestampPositionFrames();
            if (Math.abs(timestampSystemTimeUs - j) > 5000000) {
                ...
                audioTimestampPoller3.rejectTimestamp();
                ...
                audioTimestampPoller3.acceptTimestamp();
            }
        }
    }
    ...
    private void maybeUpdateLatency(long j) {
        ...
        if (this.isOutputPcm && (method = this.getLatencyMethod) != null && j - this.lastLatencySampleTimeUs >= 500000) {
            try {
                ...
                long intValue = (((long) num.intValue()) * 1000) - this.bufferSizeUs;
    ...
    private boolean forceHasPendingData() {
        if (this.needsPassthroughWorkarounds) {
            AudioTrack audioTrack2 = this.audioTrack;
            Assertions.checkNotNull(audioTrack2);
            return audioTrack2.getPlayState() == 2 && getPlaybackHeadPosition() == 0;
        }
    }
    ...
    private long getPlaybackHeadPosition() {
        ...
        long playbackHeadPosition = 4294967295L & ((long) audioTrack3.getPlaybackHeadPosition());
        if (this.needsPassthroughWorkarounds) {
