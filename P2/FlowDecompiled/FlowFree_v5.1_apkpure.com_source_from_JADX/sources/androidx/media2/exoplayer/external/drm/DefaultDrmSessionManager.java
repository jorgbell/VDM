...
public class DefaultDrmSessionManager<T extends ExoMediaCrypto> implements DrmSessionManager<T>, DefaultDrmSession.ProvisioningManager<T> {
    ...
    public boolean canAcquireSession(DrmInitData drmInitData) {
        ...
        if (getSchemeDatas(drmInitData, this.uuid, true).isEmpty()) {
            if (drmInitData.schemeDataCount != 1 || !drmInitData.get(0).matches(C0200C.COMMON_PSSH_UUID)) {
                return false;
            }
            ...
            sb.append(valueOf);
            Log.m16w("DefaultDrmSessionMgr", sb.toString());
        }
    ...
    private static List<DrmInitData.SchemeData> getSchemeDatas(DrmInitData drmInitData, UUID uuid2, boolean z) {
        ...
        for (int i = 0; i < drmInitData.schemeDataCount; i++) {
            ...
            if ((schemeData.matches(uuid2) || (C0200C.CLEARKEY_UUID.equals(uuid2) && schemeData.matches(C0200C.COMMON_PSSH_UUID))) && (schemeData.data != null || z)) {
