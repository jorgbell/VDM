...
public abstract class BinarySearchSeeker {
    ...
    public int handlePendingSeek(ExtractorInput extractorInput, PositionHolder positionHolder, OutputFrameHolder outputFrameHolder) throws InterruptedException, IOException {
        TimestampSeeker timestampSeeker2 = this.timestampSeeker;
        Assertions.checkNotNull(timestampSeeker2);
        TimestampSeeker timestampSeeker3 = timestampSeeker2;
        while (true) {
            SeekOperationParams seekOperationParams2 = this.seekOperationParams;
            Assertions.checkNotNull(seekOperationParams2);
            SeekOperationParams seekOperationParams3 = seekOperationParams2;
            long access$100 = seekOperationParams3.getFloorBytePosition();
            long access$200 = seekOperationParams3.getCeilingBytePosition();
            long access$300 = seekOperationParams3.getNextSearchBytePosition();
            if (access$200 - access$100 <= ((long) this.minimumSearchRange)) {
                ...
                TimestampSearchResult searchForTimestamp = timestampSeeker3.searchForTimestamp(extractorInput, seekOperationParams3.getTargetTimePosition(), outputFrameHolder);
                int access$500 = searchForTimestamp.type;
                if (access$500 == -3) {
                    markSeekOperationFinished(false, access$300);
                    return seekToPosition(extractorInput, access$300, positionHolder);
                } else if (access$500 == -2) {
                    seekOperationParams3.updateSeekFloor(searchForTimestamp.timestampToUpdate, searchForTimestamp.bytePositionToUpdate);
                } else if (access$500 == -1) {
                    seekOperationParams3.updateSeekCeiling(searchForTimestamp.timestampToUpdate, searchForTimestamp.bytePositionToUpdate);
    ...
    public SeekOperationParams createSeekParamsForTargetTimeUs(long j) {
        long j2 = j;
        this.seekMap.timeUsToTargetTime(j2);
        return new SeekOperationParams(j, j2, this.seekMap.floorTimePosition, this.seekMap.ceilingTimePosition, this.seekMap.floorBytePosition, this.seekMap.ceilingBytePosition, this.seekMap.approxBytesPerFrame);
    }
    ...
    public static class BinarySearchSeekMap implements SeekMap {
        ...
        public SeekMap.SeekPoints getSeekPoints(long j) {
            this.seekTimestampConverter.timeUsToTargetTime(j);
            return new SeekMap.SeekPoints(new SeekPoint(j, SeekOperationParams.calculateNextSearchBytePosition(j, this.floorTimePosition, this.ceilingTimePosition, this.floorBytePosition, this.ceilingBytePosition, this.approxBytesPerFrame)));
        }
