...
public final class HlsMediaPeriod implements MediaPeriod, HlsSampleStreamWrapper.Callback, HlsPlaylistTracker.PlaylistEventListener {
    ...
    private void buildAndPrepareSampleStreamWrappers(long j) {
        ...
        Assertions.checkNotNull(masterPlaylist);
        if (this.useSessionKeys) {
            map = deriveOverridingDrmInitData(masterPlaylist.sessionKeyDrmInitData);
        } else {
            map = Collections.emptyMap();
        }
        Map<String, DrmInitData> map2 = map;
        boolean z = !masterPlaylist.variants.isEmpty();
        List<HlsMasterPlaylist.Rendition> list = masterPlaylist.audios;
        List<HlsMasterPlaylist.Rendition> list2 = masterPlaylist.subtitles;
        this.pendingPrepareCount = 0;
        ArrayList arrayList = new ArrayList();
        ArrayList arrayList2 = new ArrayList();
        if (z) {
            buildAndPrepareMainSampleStreamWrapper(masterPlaylist, j, arrayList, arrayList2, map2);
        }
    ...
    private static Format deriveVideoFormat(Format format) {
        ...
        return Format.createVideoContainerFormat(format.f6id, format.label, format.containerMimeType, MimeTypes.getMediaMimeType(codecsOfType), codecsOfType, format.bitrate, format.width, format.height, format.frameRate, (List<byte[]>) null, format.selectionFlags, format.roleFlags);
    }
    ...
    private static Format deriveAudioFormat(Format format, Format format2, boolean z) {
        ...
        return Format.createAudioContainerFormat(format3.f6id, str3, format3.containerMimeType, MimeTypes.getMediaMimeType(str2), str2, z ? format3.bitrate : -1, i3, -1, (List<byte[]>) null, i2, i, str);
    }
}
