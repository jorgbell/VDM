        ...
        for (WorkSpec workSpec : workSpecArr) {
            if (workSpec.state == WorkInfo$State.ENQUEUED && !workSpec.isPeriodic() && workSpec.initialDelay == 0 && !workSpec.isBackedOff()) {
                if (!workSpec.hasConstraints()) {
                    Logger.get().debug(TAG, String.format("Starting work for %s", new Object[]{workSpec.f21id}), new Throwable[0]);
                    this.mWorkManagerImpl.startWork(workSpec.f21id);
                } else if (Build.VERSION.SDK_INT < 24 || !workSpec.constraints.hasContentUriTriggers()) {
                    arrayList.add(workSpec);
                    arrayList2.add(workSpec.f21id);
                }
            }
        }
    ...
    private void removeConstraintTrackingFor(String str) {
        synchronized (this.mLock) {
            ...
            while (true) {
                if (i >= size) {
                    break;
                } else if (this.mConstrainedWorkSpecs.get(i).f21id.equals(str)) {
