...
public final class zzebr {
    ...
    public static PendingIntent zza(Context context, int i, Intent intent, int i2, int i3) {
        boolean z = true;
        zzecl.zza((i2 & 95) == 0, "Cannot set any dangerous parts of intent to be mutable.");
        zzecl.zza(intent.getComponent() != null, "Must set component on Intent.");
        if (zzb(0, 1)) {
            zzecl.zza(!zzb(i2, 67108864), "Cannot set mutability flags if PendingIntent.FLAG_IMMUTABLE is set.");
        } else {
            if (Build.VERSION.SDK_INT >= 23 && !zzb(i2, 67108864)) {
                z = false;
            }
            zzecl.zza(z, "Must set PendingIntent.FLAG_IMMUTABLE for SDK >= 23 if no parts of intent are mutable.");
        }
