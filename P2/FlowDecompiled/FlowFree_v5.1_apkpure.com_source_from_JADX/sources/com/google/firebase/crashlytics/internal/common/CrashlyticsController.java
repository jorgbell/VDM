...
class CrashlyticsController {
    ...
    public synchronized void handleUncaughtException(SettingsDataProvider settingsDataProvider, Thread thread, Throwable th) {
        ...
        logger.mo22331d("Handling uncaught exception \"" + th + "\" from thread " + thread.getName());
        ...
        try {
            Utils.awaitEvenIfOnMainThread(this.backgroundWorker.submitTask(new Callable<Task<Void>>() {
                public Task<Void> call() throws Exception {
                    ...
                    if (access$100 == null) {
                        Logger.getLogger().mo22333e("Tried to write a fatal exception while no session was open.");
                    ...
                    return settingsDataProvider2.getAppSettings().onSuccessTask(executor, new SuccessContinuation<AppSettingsData, Void>() {
                        public Task<Void> then(AppSettingsData appSettingsData) throws Exception {
                            if (appSettingsData == null) {
                                Logger.getLogger().mo22339w("Received null app settings, cannot send reports at crash time.");
                                return Tasks.forResult(null);
                            }
            ...
            Logger.getLogger().mo22334e("Error handling uncaught exception", e);
        }
        return;
    }
    ...
    private Task<Boolean> waitForReportAction() {
        if (this.dataCollectionArbiter.isAutomaticDataCollectionEnabled()) {
            Logger.getLogger().mo22331d("Automatic data collection is enabled. Allowing upload.");
            this.unsentReportsAvailable.trySetResult(Boolean.FALSE);
            return Tasks.forResult(Boolean.TRUE);
        }
        Logger.getLogger().mo22331d("Automatic data collection is disabled.");
        Logger.getLogger().mo22337v("Notifying that unsent reports are available.");
        this.unsentReportsAvailable.trySetResult(Boolean.TRUE);
        Task<TContinuationResult> onSuccessTask = this.dataCollectionArbiter.waitForAutomaticDataCollectionEnabled().onSuccessTask(new SuccessContinuation<Void, Boolean>(this) {
            public Task<Boolean> then(Void voidR) throws Exception {
                return Tasks.forResult(Boolean.TRUE);
            }
        });
        Logger.getLogger().mo22331d("Waiting for send/deleteUnsentReports to be called.");
    ...
    public boolean didCrashOnPreviousExecution() {
        ...
        Logger.getLogger().mo22337v("Found previous crash marker.");
    ...
    public Task<Boolean> checkForUnsentReports() {
        ...
        Logger.getLogger().mo22339w("checkForUnsentReports should only be called once per execution.");
    ...
    public Task<Void> submitAllReports(final Task<AppSettingsData> task) {
        if (!this.reportingCoordinator.hasReportsToSend()) {
            Logger.getLogger().mo22337v("No crash reports are available to be sent.");
            this.unsentReportsAvailable.trySetResult(Boolean.FALSE);
            return Tasks.forResult(null);
        }
        Logger.getLogger().mo22337v("Crash reports are available to be sent.");
                ...
                return CrashlyticsController.this.backgroundWorker.submitTask(new Callable<Task<Void>>() {
                    public Task<Void> call() throws Exception {
                        if (!bool.booleanValue()) {
                            Logger.getLogger().mo22337v("Deleting cached crash reports...");
                            CrashlyticsController.deleteFiles(CrashlyticsController.this.listAppExceptionMarkerFiles());
                            CrashlyticsController.this.reportingCoordinator.removeAllReports();
                            CrashlyticsController.this.unsentReportsHandled.trySetResult(null);
                            return Tasks.forResult(null);
                        }
                        Logger.getLogger().mo22331d("Sending cached crash reports...");
                        ...
                        return task.onSuccessTask(executor, new SuccessContinuation<AppSettingsData, Void>() {
                            public Task<Void> then(AppSettingsData appSettingsData) throws Exception {
                                if (appSettingsData == null) {
                                    Logger.getLogger().mo22339w("Received null app settings at app startup. Cannot send cached reports");
                                    return Tasks.forResult(null);
                                }
        ...
        this.backgroundWorker.submit((Runnable) new Runnable() {
            public void run() {
                if (!CrashlyticsController.this.isHandlingException()) {
                    ...
                    String access$100 = CrashlyticsController.this.getCurrentSessionId();
                    if (access$100 == null) {
                        Logger.getLogger().mo22339w("Tried to write a non-fatal exception while no session was open.");
                    } else {
    ...
    public void setCustomKey(String str, String str2) {
        try {
            ...
            if (context2 == null || !CommonUtils.isAppDebuggable(context2)) {
                Logger.getLogger().mo22333e("Attempting to set custom attribute with null key, ignoring.");
                return;
            }
            throw e;
        }
    ...
    private void cacheUserData(final UserMetadata userMetadata2) {
        this.backgroundWorker.submit(new Callable<Void>() {
            public Void call() throws Exception {
                String access$100 = CrashlyticsController.this.getCurrentSessionId();
                if (access$100 == null) {
                    Logger.getLogger().mo22331d("Tried to cache user data while no session was open.");
                    return null;
                }
    ...
    public boolean finalizeSessions() {
        ...
        if (isHandlingException()) {
            Logger.getLogger().mo22339w("Skipping session finalization because a crash has already occurred.");
            return false;
        }
        Logger.getLogger().mo22337v("Finalizing previously open sessions.");
        try {
            doCloseSessions(true);
            Logger.getLogger().mo22337v("Closed all previously open sessions.");
            return true;
        } catch (Exception e) {
            Logger.getLogger().mo22334e("Unable to finalize previously open sessions.", e);
            return false;
        }
    }
    ...
    public void doOpenSession() {
        ...
        Logger logger = Logger.getLogger();
        logger.mo22331d("Opening a new session with ID " + clsuuid);
    ...
    private void finalizePreviousNativeSession(String str) {
        Logger logger = Logger.getLogger();
        logger.mo22337v("Finalizing native report for session " + str);
        NativeSessionFileProvider sessionFileProvider = this.nativeComponent.getSessionFileProvider(str);
        File minidumpFile = sessionFileProvider.getMinidumpFile();
        if (minidumpFile == null || !minidumpFile.exists()) {
            Logger logger2 = Logger.getLogger();
            logger2.mo22339w("No minidump data found for session " + str);
            return;
        }
        ...
        if (!file.mkdirs()) {
            Logger.getLogger().mo22339w("Couldn't create directory to store native session files, aborting.");
            return;
        }
    ...
    public void doWriteAppExceptionMarker(long j) {
        try {
            ...
            Logger.getLogger().mo22340w("Could not create app exception marker file.", e);
        }
    }

    private void writeBeginSession(String str, long j) {
        this.nativeComponent.writeBeginSession(str, String.format(Locale.US, "Crashlytics Android SDK/%s", new Object[]{CrashlyticsCore.getVersion()}), j);
    }
    ...
    private void writeSessionApp(String str) {
        ...
        String str2 = str;
        this.nativeComponent.writeSessionApp(str2, appIdentifier, appData2.versionCode, appData2.versionName, this.idManager.getCrashlyticsInstallId(), DeliveryMechanism.determineFrom(this.appData.installerPackageName).getId(), this.unityVersion);
    }
    ...
    public Task<Void> logAnalyticsAppExceptionEvents() {
        ...
        for (File file : listAppExceptionMarkerFiles()) {
            try {
                ...
                Logger.getLogger().mo22339w("Could not parse app exception timestamp from file " + file.getName());
            }
            file.delete();
        }
    ...
    private Task<Void> logAnalyticsAppExceptionEvent(final long j) {
        if (firebaseCrashExists()) {
            Logger.getLogger().mo22339w("Skipping logging Crashlytics event to Firebase, FirebaseCrash exists");
            return Tasks.forResult(null);
        }
        Logger.getLogger().mo22331d("Logging app exception event to Firebase Analytics");
