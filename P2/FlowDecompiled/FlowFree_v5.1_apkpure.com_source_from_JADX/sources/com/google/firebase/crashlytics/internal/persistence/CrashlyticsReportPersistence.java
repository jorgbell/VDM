...
public class CrashlyticsReportPersistence {
    ...
    public void persistReport(CrashlyticsReport crashlyticsReport) {
        ...
        if (session == null) {
            Logger.getLogger().mo22331d("Could not get session for report");
            return;
        }
        String identifier = session.getIdentifier();
        try {
            File sessionDirectoryById = getSessionDirectoryById(identifier);
            prepareDirectory(sessionDirectoryById);
            writeTextFile(new File(sessionDirectoryById, "report"), TRANSFORM.reportToJson(crashlyticsReport));
        } catch (IOException e) {
            Logger logger = Logger.getLogger();
            logger.mo22332d("Could not persist report for session " + identifier, e);
        }
    }
    ...
    public void persistEvent(CrashlyticsReport.Session.Event event, String str, boolean z) {
        ...
        try {
            ...
            Logger logger = Logger.getLogger();
            logger.mo22340w("Could not persist event for session " + str, e);
        }
    ...
    public void persistUserIdForSession(String str, String str2) {
        try {
            ...
            Logger logger = Logger.getLogger();
            logger.mo22340w("Could not persist user ID for session " + str2, e);
        }
    }
    ...
    public void finalizeReports(String str, long j) {
        for (File next : capAndGetOpenSessions(str)) {
            Logger logger = Logger.getLogger();
            logger.mo22337v("Finalizing report for session " + next.getName());
    ...
    public List<CrashlyticsReportWithSessionId> loadFinalizedReports() {
        ...
        for (File next : getAllFinalizedReportFiles()) {
            try {
                ...
                Logger logger = Logger.getLogger();
                logger.mo22340w("Could not load report file " + next + "; deleting", e);
                next.delete();
            }
        }
        return arrayList;
    ...
    private void synthesizeReport(File file, long j) {
        ...
        if (filesInDirectory.isEmpty()) {
            Logger logger = Logger.getLogger();
            logger.mo22337v("Session " + file.getName() + " has no events.");
            return;
        }
        ...
        while (true) {
            ...
            while (true) {
                ...
                try {
                    ...
                    logger2.mo22340w("Could not add event to report for " + next, e);
                }
            }
        }
        if (arrayList.isEmpty()) {
            Logger logger3 = Logger.getLogger();
            logger3.mo22339w("Could not parse event files for session " + file.getName());
            return;
        }
        String str = null;
        ...
        if (file2.isFile()) {
            try {
                ...
                logger4.mo22340w("Could not read user ID file in " + file.getName(), e2);
            }
        }
    ...
    private static void synthesizeNativeReportFile(File file, File file2, CrashlyticsReport.FilesPayload filesPayload, String str) {
        try {
            ...
            CrashlyticsReport withNdkPayload = crashlyticsReportJsonTransform.reportFromJson(readTextFile(file)).withNdkPayload(filesPayload);
            prepareDirectory(file2);
            writeTextFile(new File(file2, str), crashlyticsReportJsonTransform.reportToJson(withNdkPayload));
        } catch (IOException e) {
            Logger logger = Logger.getLogger();
            logger.mo22340w("Could not synthesize final native report file for " + file, e);
        }
    }
    ...
    private static void synthesizeReportFile(File file, File file2, List<CrashlyticsReport.Session.Event> list, long j, boolean z, String str) {
        try {
            ...
            if (session != null) {
                prepareDirectory(file2);
                writeTextFile(new File(file2, session.getIdentifier()), crashlyticsReportJsonTransform.reportToJson(withEvents));
            }
        } catch (IOException e) {
            Logger logger = Logger.getLogger();
            logger.mo22340w("Could not synthesize final report file for " + file, e);
        }
    }
