...
public class Intents {
    ...
    public static boolean canLaunchApplicationUrl(Context context, Uri uri) {
        ...
        if (!STORE_SCHEME_TO_URL_MAP.containsKey(intent.getScheme()) || intent.getData() == null || TextUtils.isEmpty(intent.getData().getQuery())) {
            return false;
        }
        return true;
    }
    ...
    public static void launchApplicationUrl(Context context, Uri uri) throws IntentNotResolvableException {
        ...
        launchApplicationIntent(context, new Intent("android.intent.action.VIEW", Uri.parse(String.format(map.get(intent.getScheme()), new Object[]{intent.getData().getQuery()}))));
    }
